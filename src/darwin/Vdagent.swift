...
let DEFAULT_VDAGENTD_SOCKET = "/var/run/spice-vdagent-sock"

typealias VdagentConnection = OpaquePointer

// Minimal hand-rolled CLI parser. This replaces the original's dependency on
// the swift-argument-parser SwiftPM package, which pulled in a network fetch
// at build time that Homebrew formulae must avoid. Behavior/flags are
// unchanged from upstream: -d/--debug, -S/--vdagentd-socket, -x/--foreground.
struct Vdagent {
    var debug = false
    var socketPath = DEFAULT_VDAGENTD_SOCKET
    var foreground = false

    static func parseOrExit() -> Vdagent {
        var opts = Vdagent()
        ...
        while i < args.count {
            ...
            case "-S", "--vdagentd-socket":
                i += 1
                guard i < args.count else {
                    FileHandle.standardError.write("Error: missing value for \(args[i - 1])\n".data(using: .utf8)!)
                    exit(64)
                }
                opts.socketPath = args[i]
            case "-h", "--help":
                print("Usage: spice-vdagent [-d] [-x] [-S path]")
                print("  -d, --debug                Enable debug")
                print("  -S, --vdagentd-socket path Set spice-vdagentd socket (\(DEFAULT_VDAGENTD_SOCKET))")
                print("  -x, --foreground           Do not daemonize the agent")
                exit(0)
            default:
    ...
    func fail(withError error: Error) -> Never {
...
// Entry point (replaces the @main attribute that ParsableCommand provided).
Vdagent.parseOrExit().run()
